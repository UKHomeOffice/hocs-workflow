...
public class AllocatedAspect {
    ...
    public Object validateUserAccess(ProceedingJoinPoint joinPoint, Allocated allocated) throws Throwable {
        ...
        if (!(joinPoint.getArgs()[0] instanceof UUID caseUUID && joinPoint.getArgs()[1] instanceof UUID stageUUID)) {
            throw new SecurityExceptions.PermissionCheckException(
                "Unable parse method parameters (allocated) for types [" + joinPoint.getArgs()[0].getClass().getName() + "," + joinPoint.getArgs()[1].getClass().getName() + "]",
    ...
    private void verifyAllocatedToUser(UUID caseUUID, UUID stageUUID) {
        ...
        if (!userId.equals(assignedUser)) {
            throw new SecurityExceptions.StageNotAssignedToLoggedInUserException(
                "Stage " + stageUUID.toString() + " is assigned to " + assignedUser,
    ...
    private void verifyAllocatedToTeam(UUID caseUUID, UUID stageUUID) {
        ...
        if (!teams.contains(assignedTeam)) {
            throw new SecurityExceptions.StageNotAssignedToUserTeamException(
                "Stage " + stageUUID.toString() + " is assigned to " + assignedTeam,
