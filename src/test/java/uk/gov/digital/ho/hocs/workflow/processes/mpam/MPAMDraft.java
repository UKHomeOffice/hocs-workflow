...
@Deployment(resources = "processes/MPAM/MPAM_DRAFT.bpmn")
public class MPAMDraft extends MPAMCommonTests {
    ...
    public void whenDispatch_thenUpdateTeam_andClearRejected() {
        ...
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(
            task -> task.complete(withVariables("valid", true, "DIRECTION", "FORWARD", "DraftStatus", "Dispatch")));

        Scenario.run(processScenario).startByKey("MPAM_DRAFT").execute();

        verify(processScenario).hasCompleted("Service_UpdateTeamForDispatch");
        verify(bpmnService).updateTeamByStageAndTexts(any(), any(), eq("MPAM_DISPATCH"), eq("QueueTeamUUID"),
    ...
    public void whenOfficialChangedToMinisterial_thenMinisterialValuesAreNotCleared() {
        ...
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "UpdateRefType", "RefType", "Official", "RefTypeCorrection",
                "Correction")));
        when(processScenario.waitsAtUserTask("Validate_ReferenceTypeToMinisterial_0zaqr6s")).thenReturn(
    ...
    public void whenMinisterialChangedToOfficial_thenMinisterialValuesAreCleared() {
        ...
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "UpdateRefType", "RefType", "Ministerial", "RefTypeCorrection",
                "Correction")));
        when(processScenario.waitsAtUserTask("Validate_RefTypeToOfficial_08l10xy")).thenReturn(
            task -> task.complete(withVariables("valid", true, "DIRECTION", "FORWARD")));
    ...
    public void whenPutOnCampaign_thenUpdateTeam_andClearRejected() {
        ...
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "DraftStatus", "PutOnCampaign")));
    ...
    public void whenReturnToTriage_thenCasenoteCreated_andMinisterialValuesAreCleared_andUpdatesTeam() {
        ...
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "DraftStatus", "ReturnToTriage")));
        when(processScenario.waitsAtUserTask("Validate_ReturnToTriageInput")).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "CaseNote_DraftReturnToTriage", "Case note")));
    ...
    public void whenQa_thenUpdateTeam_andClearRejected() {
        ...
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(
            task -> task.complete(withVariables("valid", true, "DIRECTION", "FORWARD", "DraftStatus", "QA")));
    ...
    public void whenTriageChangeBusinessArea_thenBusAreaStatusIsConfirmed() {
        whenChangeBusinessArea_thenBusAreaStatusIsConfirmed("MPAM_DRAFT", "Service_UpdateTeamForDraft", "MPAM_DRAFT",
            "EndEvent_MpamDraft", processScenario, bpmnService);
    }
