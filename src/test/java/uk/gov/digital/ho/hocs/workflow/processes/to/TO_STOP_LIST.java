...
@Deployment(resources = { "processes/TO/TO_STOP_LIST.bpmn", "processes/TO/TO_ENQUIRY_SUBJECT_REASON.bpmn",
...
public class TO_STOP_LIST {
    ...
    public void shouldSetEnquiryAndComplete() {
        ...
        when(TOProcess.waitsAtUserTask(STOP_LIST_INPUT)).thenReturn(
            task -> task.complete(withVariables(DIRECTION, SET_ENQUIRY))).thenReturn(task -> task.complete(
            withVariables(DIRECTION, FORWARD, "StopListOutcome", TO_DRAFT, TRIAGE_OUTCOME, TO_DRAFT)));

        whenAtCallActivity(TO_ENQUIRY_SUBJECT_REASON).deploy(rule);

        Scenario.run(TOProcess).startByKey("TO_STOP_LIST").execute();

        verify(TOProcess, times(2)).hasCompleted(STOP_LIST_INPUT);
    ...
    public void shouldSetEnquiryAndSaveAndComplete() {
        ...
        when(TOProcess.waitsAtUserTask(STOP_LIST_INPUT)).thenReturn(
            ...
            task -> task.complete(withVariables(DIRECTION, FORWARD, "StopListOutcome", SAVE))).thenReturn(
            task -> task.complete(
                withVariables(DIRECTION, FORWARD, "StopListOutcome", TO_DRAFT, TRIAGE_OUTCOME, TO_DRAFT)));
        ...
        verify(TOProcess, times(3)).hasCompleted(STOP_LIST_INPUT);
    ...
    public void shouldChangeBusinessAreaAndComplete() {
        ...
        verify(TOProcess, times(1)).hasCompleted(STOP_LIST_INPUT);
