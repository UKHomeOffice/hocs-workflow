...
public class TO_DATA_INPUT {
    ...
    public void UKVITeamCaseHappy() {
        ...
        when(TOProcess.waitsAtUserTask(CAPTURE_BUSINESS_AREA)).thenReturn(
            task -> task.complete(withVariables("BusinessArea", "UKVI")));

        when(TOProcess.waitsAtUserTask(CAPTURE_CORRESPONDENT_DETAILS)).thenReturn(
            task -> task.complete(withVariables("DIRECTION", "FORWARD")));

        when(TOProcess.waitsAtUserTask(ADD_RECIPIENT)).thenReturn(
            task -> task.complete(withVariables("DIRECTION", "FORWARD", "RecipientAdded", "YES")));

        Scenario.run(TOProcess).startByKey("TO_DATA_INPUT").execute();

        verify(TOProcess, times(1)).hasCompleted(CAPTURE_BUSINESS_AREA);
        ...
        verify(TOProcess, times(1)).hasCompleted(CAPTURE_CORRESPONDENT_DETAILS);

        verify(TOProcess, times(1)).hasCompleted(SAVE_CORRESPONDENT_DETAILS);

        verify(TOProcess, times(1)).hasCompleted(ADD_RECIPIENT);
    ...
    public void UKVITeamCaseHappyNoRecipient() {
        ...
        when(TOProcess.waitsAtUserTask(ADD_RECIPIENT)).thenReturn(
            task -> task.complete(withVariables("DIRECTION", "FORWARD", "RecipientAdded", "NO")));
    ...
    public void BFTeamCaseHappy() {
        ...
        when(TOProcess.waitsAtUserTask(CAPTURE_BUSINESS_AREA)).thenReturn(
            task -> task.complete(withVariables("BusinessArea", "BF")));
    ...
    public void HMPOTeamCaseHappy() {
        ...
        when(TOProcess.waitsAtUserTask(CAPTURE_BUSINESS_AREA)).thenReturn(
            task -> task.complete(withVariables("BusinessArea", "HMPO")));
    ...
    public void UKVITeamCaseAllBackLinks() {
        ...
        verify(TOProcess, times(2)).hasCompleted(CAPTURE_BUSINESS_AREA);

        verify(TOProcess, times(2)).hasCompleted(ALLOCATE_TO_UKVI_TEAM);

        verify(TOProcess, times(0)).hasCompleted(ALLOCATE_TO_BF_TEAM);

        verify(TOProcess, times(0)).hasCompleted(ALLOCATE_TO_HMPO_TEAM);

        verify(TOProcess, times(3)).hasCompleted(CAPTURE_CORRESPONDENT_DETAILS);

        verify(TOProcess, times(2)).hasCompleted(SAVE_CORRESPONDENT_DETAILS);

        verify(TOProcess, times(2)).hasCompleted(ADD_RECIPIENT);
