...
public class MPAMQa {
    ...
    public void whenRejectDraft_thenSavesCasenote_andUpdatesRejected_andUpdatesTeam() {
        ...
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(
            task -> task.complete(withVariables("valid", true, "DIRECTION", "FORWARD", "QaStatus", "Reject-Draft")));
        when(processScenario.waitsAtUserTask("Validate_RejectToDraft")).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "BACKWARD", "CaseNote_RejectDraft",
                "Casenote Reject"))).thenReturn(task -> task.complete(
            withVariables("valid", false, "DIRECTION", "FORWARD", "CaseNote_RejectDraft",
                "Casenote Reject"))).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "CaseNote_RejectDraft", "Casenote Reject")));

        Scenario.run(processScenario).startByKey("MPAM_QA").execute();
    ...
    public void whenRejectTriage_thenSavesCasenote_andUpdatesRejected_andUpdatesTeam() {
        ...
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(task -> task.complete(
            withVariables("valid", false, "DIRECTION", "FORWARD", "QaStatus", "Reject-Triage"))).thenReturn(
            task -> task.complete(withVariables("valid", true, "DIRECTION", "FORWARD", "QaStatus", "Reject-Triage")));
        when(processScenario.waitsAtUserTask("Validate_RejectToTriage")).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "BACKWARD", "CaseNote_RejectTriage",
                "Casenote Reject"))).thenReturn(task -> task.complete(
            withVariables("valid", false, "DIRECTION", "FORWARD", "CaseNote_RejectTriage",
                "Casenote Reject"))).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "CaseNote_RejectTriage", "Casenote Reject")));
    ...
    public void whenRequestSecretariatClearance_thenShowClearanceRequestScreen() {
        ...
        when(processScenario.waitsAtUserTask("Validate_UserInput_Clearance")).thenReturn(
            task -> task.complete(withVariables("valid", false))).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "QaStatus", "RequestSecretariatClearance")));

        when(processScenario.waitsAtUserTask("validate_request_clearance_input")).thenReturn(
    ...
    public void whenGoBackFromClearanceRequest_thenRestartDiagram() {
        ...
        when(processScenario.waitsAtUserTask("Validate_UserInput_Clearance")).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "QaStatus", "RequestSecretariatClearance")));
    ...
    public void whenPutOnCampaign_MoveToCampaignTeam() {
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(
            task -> task.complete(withVariables("valid", true, "DIRECTION", "FORWARD", "QaStatus", "PutOnCampaign")));

        when(processScenario.waitsAtUserTask("UserTask_1rbr6sp")).thenReturn(
    ...
    public void whenEscalate_UpdateAllocationNote() {
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(
            task -> task.complete(withVariables("valid", true, "DIRECTION", "FORWARD", "QaStatus", "Escalate")));

        when(processScenario.waitsAtUserTask("UserTask_1u19uk3")).thenReturn(
            task -> task.complete(withVariables("valid", false, "DIRECTION", "BACKWARD"))).thenReturn(
            task -> task.complete(withVariables("valid", false, "DIRECTION", "FORWARD"))).thenReturn(
            task -> task.complete(withVariables("valid", true, "DIRECTION", "FORWARD")));
    ...
    public void whenCloseCase_CreateCaseNote() {
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "QaStatus", "Pending"))).thenReturn(
            task -> task.complete(
                withVariables("valid", true, "DIRECTION", "FORWARD", "QaStatus", "CloseCaseTelephone")));

        when(processScenario.waitsAtUserTask("Activity_0oiigje")).thenReturn(
            task -> task.complete(withVariables("valid", false, "DIRECTION", "BACKWARD"))).thenReturn(
            task -> task.complete(withVariables("valid", false, "DIRECTION", "FORWARD"))).thenReturn(
            task -> task.complete(withVariables("valid", true, "DIRECTION", "FORWARD")));
    ...
    public void whenApproveOfficial_UpdateTeamToDispatch() {
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "QaStatus", "Approve", "RefType", "Official")));
    ...
    public void whenApproveMinisterial_UpdateTeamToPO() {
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "QaStatus", "Approve", "RefType", "Ministerial")));
    ...
    public void whenDefaultExit_LeavesQuietly() {
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "QaStatus", "NoneOfTheAbove", "RefType",
                "Ministerial")));
