...
public class DCU_MIN_Markup extends DCU_MIN_DTEN_Markup_Common {
    ...
    public void faqHappyPath_POTeamIsHomeSec_With_No_OverridePOTeam() {
        when(dcuMinMarkup.waitsAtUserTask(VALIDATE_DCU_MARKUP_DECISION)).thenReturn(
            task -> task.complete(withVariables("valid", true, "MarkupDecision", "FAQ")));

        when(dcuMinMarkup.waitsAtUserTask(VALIDATE_TOPICS)).thenReturn(
            task -> task.complete(withVariables("DIRECTION", "FORWARD")));

        when(dcuMinMarkup.waitsAtUserTask(VALIDATE_DCU_ANSWERING_FAQ)).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "CaseUUID", CASE_UUID, "OverridePOTeamUUID", "",
                "POTeamUUID", HOME_SEC_OFFICE)));

        Scenario.run(dcuMinMarkup).startByKey(DCU_MIN_MARKUP).execute();
        ...
        verify(dcuMinMarkup).hasFinished(DCU_MIN_MARKUP_END);
    }
    ...
    public void faqHappyPath_OverrideTeamIsHomeSec() {
        ...
        when(dcuMinMarkup.waitsAtUserTask(VALIDATE_DCU_ANSWERING_FAQ)).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "CaseUUID", CASE_UUID, "OverridePOTeamUUID",
                HOME_SEC_OFFICE, "POTeamUUID", "not the home sec office")));
    ...
    public void faqHappyPath_NeitherPOTeamNorPOTeamIsHomeSec() {
        ...
        when(dcuMinMarkup.waitsAtUserTask(VALIDATE_DCU_ANSWERING_FAQ)).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "CaseUUID", CASE_UUID, "OverridePOTeamUUID",
                "not the home sec office", "POTeamUUID", "not the home sec office")));
    ...
    public void prHappyPath_POTeamIsHomeSec_With_No_OverridePOTeam() {
        ...
        when(dcuMinMarkup.waitsAtUserTask(VALIDATE_DCU_ANSWERING_PR)).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "CaseUUID", CASE_UUID, "OverridePOTeamUUID", "",
                "POTeamUUID", HOME_SEC_OFFICE)));
    ...
    public void nrnHappyPath_NeitherPOTeamNorPOTeamIsHomeSec() {
        when(dcuMinMarkup.waitsAtUserTask(VALIDATE_DCU_MARKUP_DECISION)).thenReturn(
            task -> task.complete(withVariables("valid", true, "MarkupDecision", "NRN")));

        when(dcuMinMarkup.waitsAtUserTask(VALIDATE_NRN_DETAILS)).thenReturn(
            task -> task.complete(withVariables("DIRECTION", "FORWARD")));
    ...
    public void ogdHappyPath_NeitherPOTeamNorPOTeamIsHomeSec() {
        when(dcuMinMarkup.waitsAtUserTask(VALIDATE_DCU_MARKUP_DECISION)).thenReturn(
            task -> task.complete(withVariables("valid", true, "MarkupDecision", "OGD")));

        when(dcuMinMarkup.waitsAtUserTask(VALIDATE_OGD_DETAILS)).thenReturn(
    ...
    public void rejHappyPath_NeitherPOTeamNorPOTeamIsHomeSec() {
        when(dcuMinMarkup.waitsAtUserTask(VALIDATE_DCU_MARKUP_DECISION)).thenReturn(
            task -> task.complete(withVariables("valid", true, "MarkupDecision", "REJ")));

        when(dcuMinMarkup.waitsAtUserTask(VALIDATE_REJ_DETAILS)).thenReturn(
            task -> task.complete(withVariables("DIRECTION", "FORWARD")));
