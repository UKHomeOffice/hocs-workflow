...
public class COMP_SERVICE_QA {
    ...
    public void testAcceptRoute() {
        when(processScenario.waitsAtUserTask("Validate_Input")).thenReturn(
            ...
            task -> task.complete(withVariables("valid", true, "CctQaResult", "Accept")));
        ...
        verify(bpmnService).updateTeamByStageAndTexts(any(), any(), eq("COMP_SERVICE_SEND"), eq("QueueTeamUUID"),
    ...
    public void testRejectRoute() {
        when(processScenario.waitsAtUserTask("Validate_Input")).thenReturn(
            task -> task.complete(withVariables("valid", false))).thenReturn(
            task -> task.complete(withVariables("valid", true, "CctQaResult", "Reject")));

        when(processScenario.waitsAtUserTask("Validate_Reject")).thenReturn(
            ...
            task -> task.complete(
                withVariables("DIRECTION", "FORWARD", "valid", true, "CaseNote_QaReject", "Rejection")));
        ...
        verify(bpmnService).updateTeamByStageAndTexts(any(), any(), eq("COMP_SERVICE_DRAFT"), eq("QueueTeamUUID"),
            eq("QueueTeamName"), eq("Stage"));
    }
    ...
    public void testRejectBackAcceptRoute() {
        when(processScenario.waitsAtUserTask("Validate_Input")).thenReturn(
            task -> task.complete(withVariables("valid", true, "CctQaResult", "Reject"))).thenReturn(
            task -> task.complete(withVariables("valid", true, "CctQaResult", "Accept")));
        ...
        verify(bpmnService, never()).updateTeamByStageAndTexts(any(), any(), eq("COMP_SERVICE_DRAFT"),
    ...
    public void testEscalateToPsu() {
        ...
        when(processScenario.waitsAtUserTask("Activity_ScreenCategorySerious")).thenReturn(
        ...
        verify(processScenario, times(2)).hasCompleted("Screen_Input");
        verify(processScenario, times(3)).hasCompleted("Activity_ScreenCategorySerious");
    }
}
