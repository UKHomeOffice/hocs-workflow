...
public class MTS_DATA_INPUT {
    ...
    public void defaultScenario() {
        ...
        when(mtsProcess.waitsAtUserTask(SET_PRIMARY_CORRESPONDENT_USER_TASK)).thenReturn(
            task -> task.complete(withVariables("valid", true)));

        when(mtsProcess.waitsAtUserTask(MTS_DI_DETAILS_USER_TASK)).thenReturn(
            task -> task.complete(withVariables("valid", true, "DIRECTION", "FORWARD")));
    }

    @Test
    public void happyPath() {
        Scenario.run(mtsProcess).startByKey("MTS_DATA_INPUT").execute();

        verify(mtsProcess).hasCompleted(SET_PRIMARY_CORRESPONDENT_SCREEN);
        verify(mtsProcess).hasCompleted(UPDATE_PRIMARY_CORRESPONDENT_SERVICE_TASK);
        verify(mtsProcess).hasCompleted(MTS_DI_DETAILS_SCREEN);
    ...
    public void whenInvalidPrimaryCorrespondentScreen_RetriesScreen() {
        ...
        verify(mtsProcess, times(2)).hasCompleted(SET_PRIMARY_CORRESPONDENT_SCREEN);
        verify(mtsProcess).hasCompleted(UPDATE_PRIMARY_CORRESPONDENT_SERVICE_TASK);
        verify(mtsProcess).hasCompleted(MTS_DI_DETAILS_SCREEN);
    ...
    public void whenInvalidDateInputScreen_RetriesScreen() {
        ...
        verify(mtsProcess).hasCompleted(SET_PRIMARY_CORRESPONDENT_SCREEN);
        verify(mtsProcess).hasCompleted(UPDATE_PRIMARY_CORRESPONDENT_SERVICE_TASK);
        verify(mtsProcess, times(2)).hasCompleted(MTS_DI_DETAILS_SCREEN);
    ...
    public void whenBackwardDateInputScreen_GoesBack() {
        ...
        verify(mtsProcess, times(2)).hasCompleted(SET_PRIMARY_CORRESPONDENT_SCREEN);
        verify(mtsProcess, times(2)).hasCompleted(UPDATE_PRIMARY_CORRESPONDENT_SERVICE_TASK);
        verify(mtsProcess, times(2)).hasCompleted(MTS_DI_DETAILS_SCREEN);
