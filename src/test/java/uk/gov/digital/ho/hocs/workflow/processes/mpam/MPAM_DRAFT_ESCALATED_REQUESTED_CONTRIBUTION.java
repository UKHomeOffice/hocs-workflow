...
public class MPAM_DRAFT_ESCALATED_REQUESTED_CONTRIBUTION {
    ...
    public void happyPath() {
        when(mpamProcess.waitsAtUserTask(DRAFT_ESCALATED_REQUESTED_CONTRIBUTION_SCREEN_USER_TASK)).thenReturn(
            task -> task.complete(
                withVariables("valid", true, "DraftEscalatedRequestedContributionOutcome", "Complete")));
        ...
        Scenario.run(mpamProcess).startByKey("MPAM_DRAFT_ESCALATED_REQUESTED_CONTRIBUTION").execute();

        verify(mpamProcess, times(1)).hasCompleted(DRAFT_ESCALATED_REQUESTED_CONTRIBUTION_SCREEN);
    ...
    public void requestContributionScreen_validFalseFirstTime() {
        when(mpamProcess.waitsAtUserTask(DRAFT_ESCALATED_REQUESTED_CONTRIBUTION_SCREEN_USER_TASK)).thenReturn(
            task -> task.complete(
                withVariables("valid", false, "DraftEscalatedRequestedContributionOutcome", "Complete"))).thenReturn(
            task -> task.complete(
                withVariables("valid", true, "DraftEscalatedRequestedContributionOutcome", "Complete")));
        ...
        verify(mpamProcess, times(2)).hasCompleted(DRAFT_ESCALATED_REQUESTED_CONTRIBUTION_SCREEN);
    ...
    public void requestContributionScreen_escalateCase() {
        when(mpamProcess.waitsAtUserTask(DRAFT_ESCALATED_REQUESTED_CONTRIBUTION_SCREEN_USER_TASK)).thenReturn(
            task -> task.complete(
                withVariables("valid", true, "DraftEscalatedRequestedContributionOutcome", "Escalate")));
    ...
    public void requestContributionScreen_putInCampaign() {
        when(mpamProcess.waitsAtUserTask(DRAFT_ESCALATED_REQUESTED_CONTRIBUTION_SCREEN_USER_TASK)).thenReturn(
            task -> task.complete(
                withVariables("valid", true, "DraftEscalatedRequestedContributionOutcome", "PutOnCampaign")));
