...
@Deployment(resources = "processes/DCU/DCU_MIN_PRIVATE_OFFICE.bpmn")
public class DCU_MIN_PRIVATE_OFFICE {
    ...
    public static final String UPDATE_STAGE_DEADLINES_FOR_MINISTER_OR_DIRECTOR_TEAMS = "UPDATE_STAGE_DEADLINES_FOR_MINISTER_OR_DIRECTOR_TEAMS";
    ...
    public void acceptHappyPath() {
        when(processScenario.waitsAtUserTask(VALIDATE_APPROVE_PRIVATE_OFFICE)).thenReturn(
            task -> task.complete(withVariables("valid", true, "PrivateOfficeDecision", "ACCEPT")));

        Scenario.run(processScenario).startByKey(DCU_MIN_PRIVATE_OFFICE).execute();
    ...
    public void changeHappyPath_POTeamIsHomeSec_With_No_PrivateOfficeOverridePOTeamUUID() {
        when(processScenario.waitsAtUserTask(VALIDATE_APPROVE_PRIVATE_OFFICE)).thenReturn(
            task -> task.complete(withVariables("valid", true, "PrivateOfficeDecision", "CHANGE")));

        when(processScenario.waitsAtUserTask(VALIDATE_CHANGE_MINISTER)).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "CaseUUID", CASE_UUID,
                "PrivateOfficeOverridePOTeamUUID", "", "POTeamUUID", HOME_SEC_OFFICE)));
    ...
    public void changeHappyPath_OverrideTeamIsHomeSec() {
        ...
        when(processScenario.waitsAtUserTask(VALIDATE_CHANGE_MINISTER)).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "CaseUUID", CASE_UUID,
                "PrivateOfficeOverridePOTeamUUID", HOME_SEC_OFFICE, "POTeamUUID", "not the home sec office")));
    ...
    public void changeHappyPath_NeitherPOTeamNorPOTeamIsHomeSec() {
        ...
        when(processScenario.waitsAtUserTask(VALIDATE_CHANGE_MINISTER)).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "CaseUUID", CASE_UUID,
                "PrivateOfficeOverridePOTeamUUID", "not the home sec office", "POTeamUUID",
                "not the home sec office")));
    ...
    public void rejectHappyPath() {
        when(processScenario.waitsAtUserTask(VALIDATE_APPROVE_PRIVATE_OFFICE)).thenReturn(
            task -> task.complete(withVariables("valid", true, "PrivateOfficeDecision", "REJECT")));

        when(processScenario.waitsAtUserTask(VALIDATE_REJECTION_NOTE)).thenReturn(
    ...
    private void verifyMinisterOrDirectorOfficeDeadlinesSet() {
        ...
        verify(bpmnService).updateDeadlineForStages(eq(CASE_UUID), any(), eq("DCU_MIN_INITIAL_DRAFT"), eq("10"),
            eq("DCU_MIN_QA_RESPONSE"), eq("10"), eq("DCU_MIN_PRIVATE_OFFICE"), eq("19"),
            eq("DCU_MIN_MINISTER_SIGN_OFF"), eq("19"), eq("DCU_MIN_TRANSFER_CONFIRMATION"), eq("20"),
            eq("DCU_MIN_NO_REPLY_NEEDED_CONFIRMATION"), eq("20"), eq("DCU_MIN_DISPATCH"), eq("20"),
    ...
    public void whenTopic_thenChangeTopic_andSaveNote_andSavePrimaryTopic() {
        ...
        when(processScenario.waitsAtUserTask("Validate_ApprovePrivateOffice")).thenReturn(
            task -> task.complete(withVariables("valid", false, "DIRECTION", "FORWARD"))).thenReturn(
            task -> task.complete(
                withVariables("valid", true, "DIRECTION", "FORWARD", "PrivateOfficeDecision", "TOPIC"))).thenReturn(
            task -> task.complete(
                withVariables("valid", true, "DIRECTION", "FORWARD", "PrivateOfficeDecision", "TOPIC")));
        when(processScenario.waitsAtUserTask("Validate_ChangeTopic")).thenReturn(
            task -> task.complete(withVariables("valid", true, "DIRECTION", "BACKWARD"))).thenReturn(
            task -> task.complete(withVariables("valid", false, "DIRECTION", "FORWARD"))).thenReturn(
            task -> task.complete(
                withVariables("valid", true, "DIRECTION", "FORWARD", "POTeamUUID", "not the home sec office",
                    "OverridePOTeamUUID", overridePOTeamUUID.toString(), "PrivateOfficeOverridePOTeamName",
                    "Minister of State for Crime and Policing", "PrivateOfficeOverridePOTeamUUID",
                    "5d335594-2f88-4b06-ab08-ad1e6d6e39aa", "PrivateOfficeOverridePOTeamUnitHistoricName", "",
                    "CaseNote_PrivateOfficeTopic", "Change topic", "Topics", topicUUID.toString())));
        ...
        verify(bpmnService).updateTeamsForPrimaryTopic(any(), any(), eq(topicUUID.toString()),
            eq("DCU_MIN_PRIVATE_OFFICE"), eq("PrivateOfficeOverridePOTeamUUID"), eq("PrivateOfficeOverridePOTeamName"),
            eq("PrivateOfficeOverridePOTeamUnitHistoricName"));
        verify(processScenario).hasCompleted("UPDATE_MINISTER_OR_DIRECTOR_DEADLINE");
        verify(processScenario).hasCompleted("UPDATE_STAGE_DEADLINES_FOR_MINISTER_OR_DIRECTOR_TEAMS");
        verify(processScenario).hasFinished("DCU_MIN_PRIVATE_OFFICE_END");
    }
