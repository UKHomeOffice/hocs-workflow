...
@Deployment(resources = { "processes/TO/TO_QA.bpmn" })
public class TO_QA {
    ...
    public void testSaveOnceThenAccept() {
        ...
        when(TOProcess.waitsAtUserTask(TO_QA_OUTCOME)).thenReturn(
            task -> task.complete(withVariables(QA_STATUS, "Save", DIRECTION, FORWARD))).thenReturn(
            task -> task.complete(withVariables(QA_STATUS, "SendToDispatch", DIRECTION, FORWARD)));

        Scenario.run(TOProcess).startByKey("TO_QA").execute();

        verify(TOProcess, times(2)).hasCompleted(TO_QA_OUTCOME);

        verify(TOProcess, times(1)).hasCompleted(CLEAR_REJ_NOTE);

        verify(TOProcess, times(1)).hasCompleted(UPDATE_APP_Approved);

        // Not invoked
        verify(TOProcess, times(0)).hasCompleted(TO_QA_REJECTION_NOTE);

        verify(TOProcess, times(0)).hasCompleted(SAVE_REJ_NOTE);

        verify(TOProcess, times(0)).hasCompleted(UPDATE_APP_Rejected);

        verify(TOProcess, times(0)).hasCompleted(UPDATE_APP_DecisionNotYetMade);
    ...
    public void shouldChangeBusinessAreaAndComplete() {
        ...
        verify(TOProcess, times(1)).hasCompleted(TO_QA_OUTCOME);
    ...
    public void testRejectSendToDraft() {
        when(TOProcess.waitsAtUserTask(TO_QA_OUTCOME)).thenReturn(
            task -> task.complete(withVariables(QA_STATUS, "SendToDraft", DIRECTION, FORWARD)));

        when(TOProcess.waitsAtUserTask(TO_QA_REJECTION_NOTE)).thenReturn(
        ...
        verify(TOProcess, times(1)).hasCompleted(TO_QA_REJECTION_NOTE);

        verify(TOProcess, times(1)).hasCompleted(SAVE_REJ_NOTE);

        verify(TOProcess, times(1)).hasCompleted(UPDATE_APP_Rejected);

        // Not invoked
        verify(TOProcess, times(0)).hasCompleted(CLEAR_REJ_NOTE);

        verify(TOProcess, times(0)).hasCompleted(UPDATE_APP_Approved);
    ...
    public void testRejectSendToTriageGoBackSentToTriage() {
        ...
        when(TOProcess.waitsAtUserTask(TO_QA_OUTCOME)).thenReturn(
            task -> task.complete(withVariables(QA_STATUS, "SendToDraft", DIRECTION, FORWARD))).thenReturn(
            task -> task.complete(withVariables(QA_STATUS, "SendToTriage", DIRECTION, FORWARD)));
        ...
        verify(TOProcess, times(2)).hasCompleted(TO_QA_REJECTION_NOTE);
    ...
    public void testPutOnStopList() {
        when(TOProcess.waitsAtUserTask(TO_QA_OUTCOME)).thenReturn(
            task -> task.complete(withVariables(QA_STATUS, "SendToStopList", DIRECTION, FORWARD)));
        ...
        verify(TOProcess, times(1)).hasCompleted(UPDATE_APP_DecisionNotYetMade);
    ...
    public void testPutOnCampaign() {
        when(TOProcess.waitsAtUserTask(TO_QA_OUTCOME)).thenReturn(
            task -> task.complete(withVariables(QA_STATUS, PUT_ON_CAMPAIGN, DIRECTION, FORWARD)));
    ...
    public void testCloseCaseBackThenCloseCaseForward() {
        when(TOProcess.waitsAtUserTask(TO_QA_OUTCOME)).thenReturn(
            task -> task.complete(withVariables(QA_STATUS, CLOSE_CASE, DIRECTION, FORWARD))).thenReturn(
            task -> task.complete(withVariables(QA_STATUS, CLOSE_CASE, DIRECTION, FORWARD)));
