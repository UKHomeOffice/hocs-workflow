...
public class DCU_MIN_MinisterSignOff {
    ...
    public void acceptSignOffScenario() {
        ...
        verify(dcuMinSignOffProcess).hasFinished(DCU_MIN_MINISTER_SIGN_OFF_END);
    ...
    public void rejectSignOffScenario() {
        ...
        when(dcuMinSignOffProcess.waitsAtUserTask(APPROVE_MINISTER_SIGN_OFF)).thenReturn(
            task -> task.complete(withVariables("valid", true, "MinisterSignOffDecision", "REJECT")));

        when(dcuMinSignOffProcess.waitsAtUserTask(VALIDATE_REJECTION_NOTE)).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "CaseNote_MinisterReject", REJECTION_REASON)));

        Scenario.run(dcuMinSignOffProcess).startByKey("DCU_MIN_MINISTER_SIGN_OFF").execute();

        verify(dcuMinSignOffProcess).hasFinished(VALIDATE_REJECTION_NOTE);
    ...
    public void notApplicableScenario() {
        ...
        when(dcuMinSignOffProcess.waitsAtUserTask(APPROVE_MINISTER_SIGN_OFF)).thenReturn(
            task -> task.complete(withVariables("valid", true, "MinisterSignOffDecision", "NOT_APPLICABLE")));

        when(dcuMinSignOffProcess.waitsAtUserTask(VALIDATE_NOT_APPLICABLE)).thenReturn(task -> task.complete(
            withVariables("valid", true, "DIRECTION", "FORWARD", "CaseNote_MinisterNotApplicable", NA_REASON)));

        Scenario.run(dcuMinSignOffProcess).startByKey("DCU_MIN_MINISTER_SIGN_OFF").execute();

        verify(dcuMinSignOffProcess).hasFinished(VALIDATE_NOT_APPLICABLE);
    ...
    public void notApplicableScenarioBackLinkGoesBackToMinisterSignOff() {
        ...
        verify(dcuMinSignOffProcess, times(2)).hasFinished(APPROVE_MINISTER_SIGN_OFF);

        verify(dcuMinSignOffProcess, times(1)).hasFinished(VALIDATE_NOT_APPLICABLE);
