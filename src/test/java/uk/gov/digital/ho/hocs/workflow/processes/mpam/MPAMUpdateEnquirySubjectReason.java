...
public class MPAMUpdateEnquirySubjectReason {
    ...
    public void whenEnquirySubjectBackPressed_thenClearCalledAndEndEvent() {
        when(processScenario.waitsAtUserTask(MPAM_ENQUIRY_SUBJECT_ACTIVITY)).thenReturn(
            task -> task.complete(withVariables("DIRECTION", "BACKWARD")));

        Scenario.run(processScenario).startByKey(MPAM_UPDATE_ENQUIRY_SUBJECT_REASON_BPMN).execute();
    ...
    public void whenEnquiryReasonBackPressed_thenClearCalledAndEndEvent() {
        ...
        when(processScenario.waitsAtUserTask(MPAM_ENQUIRY_REASON_ACTIVITY)).thenReturn(
            task -> task.complete(withVariables("DIRECTION", "BACKWARD")));
    ...
    public void whenEnquiryReasonEuNational_detailsScreen() {
        ...
        when(processScenario.waitsAtUserTask(MPAM_ENQUIRY_REASON_ACTIVITY)).thenReturn(task -> task.complete(
            withVariables("DIRECTION", "FORWARD", "valid", "true", "EnquiryReason", "EuNationalComplianceMeasures")));

        when(processScenario.waitsAtUserTask(EU_NATIONAL_DETAILS_ENQUIRY_REASON_ACTIVITY)).thenReturn(
    ...
    public void whenEnquiryReasonEuNational_detailsScreen_withOther() {
        ...
        when(processScenario.waitsAtUserTask(EU_NATIONAL_DETAILS_ENQUIRY_REASON_ACTIVITY)).thenReturn(
            task -> task.complete(withVariables("DIRECTION", "FORWARD", "ComplianceMeasuresOtherDetails", "TEST")));
