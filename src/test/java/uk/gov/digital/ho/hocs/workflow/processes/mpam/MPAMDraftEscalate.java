...
@Deployment(resources = "processes/MPAM/MPAM_DRAFT_ESCALATE.bpmn")
public class MPAMDraftEscalate extends MPAMCommonTests {
    ...
    public void whenMinisterialChangedToOfficial_thenMinisterialValuesAreCleared() {
        ...
        Scenario.run(processScenario).startByKey("MPAM_DRAFT_ESCALATE").execute();
    ...
    public void whenRequestContributions_thenRequestContributions() {
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(task -> task.complete(
            withVariables("DIRECTION", "FORWARD", "valid", true, "DraftStatus", "RequestContribution")));
    ...
    public void whenPutOnCampaign_thenPutOnCampaign() {
        when(processScenario.waitsAtUserTask("Validate_UserInput")).thenReturn(task -> task.complete(
            withVariables("DIRECTION", "FORWARD", "valid", true, "DraftStatus", "PutOnCampaign")));

        when(processScenario.waitsAtUserTask("UserTask_RequestCampaign")).thenReturn(
            task -> task.complete(withVariables("DIRECTION", "FORWARD", "valid", false))).thenReturn(
    ...
    public void whenTriageChangeBusinessArea_thenBusAreaStatusIsConfirmed() {
        whenChangeBusinessArea_thenBusAreaStatusIsConfirmed("MPAM_DRAFT_ESCALATE", "Service_UpdateTeamForDraft",
            "MPAM_DRAFT", "EndEvent_MpamDraftEscalate", processScenario, bpmnService);
    }
