...
public class TO_ENQUIRY_SUBJECT_REASON {
    ...
    public void shouldSetSubAndReason() {
        // GIVEN
        when(TOProcess.waitsAtUserTask(TO_ENQUIRY_SET_SUBJECT)).thenReturn(
            task -> task.complete(withVariables("DIRECTION", FORWARD)));

        when(TOProcess.waitsAtUserTask(TO_ENQUIRY_SET_REASON)).thenReturn(
            task -> task.complete(withVariables("DIRECTION", FORWARD)));

        // WHEN
        Scenario.run(TOProcess).startByKey("TO_ENQUIRY_SUBJECT_REASON").execute();

        // THEN
        verify(TOProcess, times(1)).hasCompleted(TO_ENQUIRY_SET_SUBJECT);

        verify(TOProcess, times(1)).hasCompleted(TO_ENQUIRY_SET_REASON);

        verify(TOProcess, times(1)).hasCompleted(SET_ENQ_SUB_REASON);

        verify(TOProcess, times(1)).hasCompleted(CLEAR_TEMP_SUB_REASON);

        verify(TOProcess, times(0)).hasCompleted(CLEAR_TEMP_SUBJ);
    ...
    public void shouldClearSubjWhenBackwardsAtSetReason() {
        ...
        // THEN
        verify(TOProcess, times(2)).hasCompleted(TO_ENQUIRY_SET_SUBJECT);

        verify(TOProcess, times(1)).hasCompleted(TO_ENQUIRY_SET_REASON);

        verify(TOProcess, times(0)).hasCompleted(SET_ENQ_SUB_REASON);

        verify(TOProcess, times(0)).hasCompleted(CLEAR_TEMP_SUB_REASON);

        verify(TOProcess, times(1)).hasCompleted(CLEAR_TEMP_SUBJ);
    ...
    public void shouldNotSetSubjOrReason() {
        ...
        verify(TOProcess, times(0)).hasCompleted(TO_ENQUIRY_SET_REASON);
